...
from .EAEmodel import TagCrossAttRoleEAEModel
...
class TagCrossAttRoleEAETrainer(BasicTrainer):
    ...
    def load_model(self, checkpoint=None):
        if checkpoint:
            ...
            self.model = TagCrossAttRoleEAEModel(self.config, self.tokenizer, self.type_set)
    ...
    def train(self, train_data, dev_data, **kwargs):
        ...
        for epoch in range(1, self.config.max_epoch+1):
            ...
            lossplot.append(np.mean(cummulate_loss))
            # eval dev
        ...
        logger.info(f"loss save in ./loss/{self.config.model_type}_{self.config.dataset}_{self.config.split}.csv")
        lossplot = pd.DataFrame(lossplot , columns= ['loss'])
        lossplot.to_csv(f"./loss/{self.config.model_type}_{self.config.dataset}_{self.config.split}.csv",index=False)
